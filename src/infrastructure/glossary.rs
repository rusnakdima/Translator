...
use chrono::Utc;
use dioxus_shared::{algo_execute, storage::SignalStore, AlgorithmRegistry};
use serde_json::json;
use std::sync::Arc;
use uuid::Uuid;
        ...
        match algo_execute(&self.registry, "search.schemas", data, None, None) {
            Ok(matching_json) => {
                let matching: Vec<String> =
                    serde_json::from_value(matching_json).unwrap_or_default();
                matching
                    .into_iter()
...
mod tests {
    ...
    impl TestGlossaryStorage {
        fn new(store: Arc<SignalStore>) -> Self {
            ...
            Self {
                inner: RefCell::new(GlossaryStorage::new(store, registry)),
            }
        }
        ...
        fn from_json_and_add(
            store: Arc<SignalStore>,
            json: serde_json::Value,
        ) -> Result<Vec<GlossaryItem>, String> {
            let items: Vec<GlossaryItem> =
                serde_json::from_value(json).map_err(|e| format!("{e}"))?;
            let registry = Arc::new(AlgorithmRegistry::new());
    ...
    fn glossary_serialization_roundtrip() {
        ...
        let items: Vec<GlossaryItem> =
            serde_json::from_value(json.clone()).expect("roundtrip should succeed");
