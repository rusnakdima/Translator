...
mod tests {
  ...
  fn language_serde_roundtrip() {
    let lang = Language { code: "fr".to_string(), name: "French".to_string() };
  ...
  fn translation_response_serde_roundtrip() {
    let resp = TranslationResponse { translated_text: "bonjour".to_string() };
    let json = serde_json::to_string(&resp).expect("should serialize");
    let roundtrip: TranslationResponse = serde_json::from_str(&json).expect("should deserialize");
