    ...
    match action.name.as_str() {
        ...
        "add_term" => {
            ...
            if !term.is_empty() && !term_translation.is_empty() {
                ...
                let GlossaryItem { id, .. } =
                    storage.add_item(term.clone(), term_translation.clone());
...
fn App(props: AppProps) -> Element {
    ...
    // main thread. The eval processing happens in use_effect (non-blocking poll),
    // while a background thread collects completed results and enqueues them.
    let desktop = dioxus_desktop::use_window();
    let bridge_state = props.bridge_state.clone();
    let shutdown = Arc::new(AtomicBool::new(false));
    let shutdown_bg = shutdown.clone();

    // Channel: main thread sends (request, response_tx) to background thread
    let (result_tx, result_rx) = channel::<(String, String)>();

    // Background thread: collects completed eval results and enqueues them
    let bridge_state_bg = bridge_state.clone();
    std::thread::spawn(move || {
        while !shutdown_bg.load(Ordering::SeqCst) {
            // Drain completed results from main thread
            while let Ok((id, result)) = result_rx.try_recv() {
                bridge_state_bg.enqueue_js_result(id, result);
            }
    ...
    let mut eval_loop_tick = use_signal(|| false);

    use_effect(move || {
        let _ = eval_loop_tick.read();
        let bridge_state = bridge_state.clone();
        let desktop = desktop.clone();
        let result_tx = result_tx.clone();

        for (id, result) in bridge_state.dequeue_js_results() {
            let _ = result_tx.send((id, result));
        }

        if let Some(request) = bridge_state.dequeue_eval_requests().into_iter().next() {
...
fn process_eval_on_main_thread(
    ...
    rx.recv_timeout(std::time::Duration::from_secs(5))
        .unwrap_or_else(|_| r#"{"error":"callback timeout or never fired"}"#.to_string())
}
